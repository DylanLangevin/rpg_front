...
let femaleCitizen = new Pnj("Villageoise", 'character_profile/female_citizen.png', [620, 126]);//position a définir
...
function pnjParcPosition() {
    ctxBackground.drawImage(femaleCitizen.character, 0, 122,frameWidth*scaleDivider, frameHeight*scaleDivider, femaleCitizen.position[0], femaleCitizen.position[1], frameWidth, frameHeight)
}

function pnjParcRightPosition() {
    ctxBackground.drawImage(engineer.character, 0, 122,frameWidth*scaleDivider, frameHeight*scaleDivider, engineer.position[0], engineer.position[1], frameWidth, frameHeight)
